...
export default function AdminAddProduct() {
  ...
  const addNewCategory = async (e) => {
    ...
    const formData = {
      ...
      DesignName: newCategory.DesignName,
      Description: newCategory.Description,
      Slug: newCategory.Slug,
      LabelCode: newCategory.LabelCode,
      Status: "Active",
      MinQuantity: newCategory.MinQuantity,
      MinWeight: newCategory.MinWeight,
    ...
    try {
      const response = await fetch(
        !newCategory.OldEntry ? a133 : a132,
        // a96,
        {
          method: "POST",
          headers: {
  ...
  const filteredProducts = allProductsList.filter(
    (product) => product.CategoryId == newCategory.CategoryId
  );
  return (
    <div>
      <AdminHeading />
                ...
                >
                  ...
                  <input
                    ...
                    onInput={(e) => {
                      ...
                      // Check if the last character is a number, if so, remove it
                      if (
                        inputValue.length > 0 &&
                      ...
                      e.target.value = inputValue.substring(0, 10); // Ensure it does not exceed 4 characters (adjusted as per your previous example)
                    }}
                  />
                  <label>Min Quantity</label>
                  <input
                    name="MinQuantity"
                    value={newCategory.MinQuantity}
                    onChange={handleNewCategoryChange}
                    type="text"
                  />
